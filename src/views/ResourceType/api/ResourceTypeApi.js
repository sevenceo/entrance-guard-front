...
export function Editor(data) {
    return fetch({
        url: 'dealer/resource-type/' + data.id,
        method: 'PATCH',
        data: data
    }).then(function (result) {
        if (result.data.code=="0") {
...
export function BatchDelete(ids) {
    return fetch({
        url: 'dealer/resource-type/delete',
        method: 'POST',
        data: ids
    }).then(function (result) {
        Message({
...
export function Status(id, type) {
    if (type == '1') {
        return fetch({
            url: 'dealer/resource-type/disable/' + id,
            method: 'GET',
        }).then(function (result) {
            Message({
                message: '禁用成功',
        ...
        return fetch({
            url: 'dealer/resource-type/enable/' + id,
            method: 'GET',
        }).then(function (result) {
            Message({
                message: '启用成功',
...
export function BatchStatus(ids, type) {
    if (type == '1') {
        return fetch({
            url: 'dealer/resource-type/disable',
            method: 'POST',
            data: ids
        }).then(function (result) {
            Message({
        ...
        return fetch({
            url: 'dealer/resource-type/enable',
            method: 'POST',
            data: ids
        }).then(function (result) {
            Message({
