...
export function BatchDelete(ids) {
    return fetch({
        url: '/human-scene-ref/delete',
        method: 'POST',
        data: ids
    }).then(function (result) {
        Message({
            message: '批量删除人、场景关系成功',
            type: 'success',
            customClass: 'msg-success',
            iconClass: 'ic'
        })
...
export function Status(id, type) {
    if (type == '1') {
        return fetch({
            url: '/human-scene-ref/disable/' + id,
            method: 'GET',
        }).then(function (result) {
            Message({
                message: '禁用成功',
