...
export function GetList(page) {
    ...
    return fetch({
        url: 'attendance/holiday/'+page.size+'/'+page.page,
        method: 'POST',
        data: page
...
export function Editor(data) {
    return fetch({
        url: 'attendance/holiday/' + data.id,
        method: 'PATCH',
        data: data
    }).then(function (result) {
        if (result) {
...
export function Status(id, type) {
    if (type == '1') {
        return fetch({
            url: 'attendance/holiday/disable/' + id,
            method: 'GET',
        }).then(function (result) {
            Message({
                message: '禁用成功',
...
export function BatchStatus(ids, type) {
    if (type == '1') {
        return fetch({
            url: 'attendance/holiday/disable',
            method: 'POST',
            data: ids
        }).then(function (result) {
            Message({
