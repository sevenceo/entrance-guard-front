...
export function BatchDelete(ids) {
    return fetch({
        url: '/resource-detail/delete',
        method: 'POST',
        data: ids
    }).then(function (result) {
        Message({
            message: '批量删除资源详细信息成功',
            type: 'success',
            customClass: 'msg-success',
            iconClass: 'ic'
        })
...
export function Status(id, type) {
    if (type == '1') {
        return fetch({
            url: '/resource-detail/disable/' + id,
            method: 'GET',
        }).then(function (result) {
            Message({
                message: '禁用成功',
