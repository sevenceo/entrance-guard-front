...
export default {
    ...
    methods: {
        ...
        getHuman(humanPageParam) {
            GetOutWhiteListHuman(humanPageParam).then((response) => {
            ...
            if (/^image/.test(file.raw.type)) {
                ...
                fileReader.onloadend = function (e) {
                    ...
                    IMG.onload = function () {
                        ...
                        self.addHumanTemp.baseStr = self.base64;
                        self.imageUrl = self.base64;
                        self.addHumanTemp.imageUrl = self.base64;
        ...
        matchHuman() {
            ...
            this.temp.humanIds = this.humanSelection.map(item => item.id);
            if (beforeMethod("whiteListAdd")) {
                Create(this.temp)
                    .then(() => {
                        this.dialogFormVisible = false;
                        this.dialogStatus = 'create';
                        this.getList(this.pageParam);
                        afterMethod("whiteListAdd");
                    })
            }
