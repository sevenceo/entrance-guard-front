...
export function GetList(page) {
    ...
    if (page) {
        params = $.extend({
            ...
            supplierCode: page.supplierCode
        }, page);
    }

    return fetch({
        url: 'dealer/supplier/page',
        method: 'POST',
        data: page
...
export function Status(id, type) {
    if (type == '1') {
        return fetch({
            url: 'dealer/supplier/disable/' + id,
            method: 'GET',
        }).then(function (result) {
            Message({
                message: '禁用成功',
