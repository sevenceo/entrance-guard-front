...
/**
  ...
  * </pre>
  */
import {GetList, Create, Editor, Delete, BatchDelete, Status, BatchStatus,GetResourceTree,GetSaasUsers,GetDataType} from "./api/operateHistoryApi";
...
export default {
            ...
            rules: {
                ...
                methodName:
                    [
                            ...
                            {max: 255, message: '操作方法不得超过255个字符'}],
                args:
                    [
                ...
                consumeTime:
                    [
                            {required: true, message: '请输入方法消耗时间', trigger: 'blur'},
],
                operator:
                    [
                            {required: true, message: '请输入操作人', trigger: 'blur'},
                            {max: 255, message: '操作人不得超过255个字符'}],

                tenantId:
                    [
                            {required: true, message: '请输入', trigger: 'blur'},
                            {max: 128, message: '不得超过128个字符'}],
            },
    ...
    methods: {
        ...
        nodeClick(data, node, object){
            ...
            this.pageParam.operator = '';
            this.operatorType = data.text;
            this.parentTree = false;
            if (data.children){
                GetSaasUsers(data.id,'org').then((result)=>{
                    this.users = result.data;
                    this.pageParam.adminId = '';
                    this.pageParam.corpId = '';
                    this.pageParam.orgId = data.id;
                })
            }else if(!data.children && data.id == 'adminId'){
                GetSaasUsers(data.id,'platform').then((result)=>{
                    this.users = result.data;
                    this.pageParam.corpId = '';
                    this.pageParam.orgId = '';
                    this.pageParam.adminId = data.id;
                })
            }else{
                GetSaasUsers(data.id,'corp').then((result)=>{
                    this.users = result.data;
                    this.pageParam.adminId = '';
                    this.pageParam.orgId = '';
